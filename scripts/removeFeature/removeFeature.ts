...
const featureState: featureStateType = process.argv[3] as featureStateType
...
if (featureState !== 'off' && featureState !== 'on') {
	throw new Error('feature state is not on or off')
}
...
const replaceToggleFunction = (node: Node) => {
	const objectOptions = node.getFirstAncestorByKind(SyntaxKind.ObjectLiteralExpression)
	...
	const onFunction = onFunctionProperty?.getFirstAncestorByKind(SyntaxKind.ArrowFunction)
	const offFunction = offFunctionProperty?.getFirstAncestorByKind(SyntaxKind.ArrowFunction)
	const featureName = featureNameProperty
		?.getFirstAncestorByKind(SyntaxKind.StringLiteral)
		?.getText()
		.slice(1, -1)
