...
import { ConnectionPlugin, SocketData, createPseudoconnection } from 'rete-connection-plugin'
...
export function useMagneticConnection<S extends Schemes, K = never>(
	...
	;(connection as ConnectionPlugin<S, K>).addPipe(async (context) => {
		...
		if (context.type === 'connectionpick') {
			...
			const nearestSockets = socketsList.filter((item) => nearestNodes.includes(item.nodeId))
