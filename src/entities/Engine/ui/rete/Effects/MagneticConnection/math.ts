...
export function findNearestPoint<T extends Position>(points: T[], target: Position, maxDistance: number) {
	return points.reduce(
		(nearestPoint, point) => {
			const distance = Math.sqrt((point.x - target.x) ** 2 + (point.y - target.y) ** 2)

			if (distance > maxDistance) return nearestPoint
			if (nearestPoint === null || distance < nearestPoint.distance) return { point, distance }
			return nearestPoint
		},
