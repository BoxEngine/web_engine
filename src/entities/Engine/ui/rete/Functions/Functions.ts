...
export class Functions<S extends Schemes> {
	...
	private async execute(inputs: Record<string, any>, editor: NodeEditor<S>, engine: DataflowEngine<S>) {
	...
	private injectInputs(nodes: S['Node'][], inputData: Record<string, any>) {
		const inputNodes = nodes.filter((node): node is InputNode => node instanceof InputNode)
	...
	private async retrieveOutputs(nodes: S['Node'][], engine: DataflowEngine<S>) {
		const outputNodes = nodes.filter((node): node is OutputNode => node instanceof OutputNode)
