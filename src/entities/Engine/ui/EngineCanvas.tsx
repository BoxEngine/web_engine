			...
			{editor && (
				<div className={classes.header}>
					<div className={classes.tabs}>
						{modules.map((module) => (
							<button key={module} onClick={() => editor.openModule(module)}>
								{module}
							</button>
						))}
					</div>
					...
					<button onClick={() => editor?.saveModule()}>Save</button>
					<button onClick={() => editor?.restoreModule()}>Restore</button>
				</div>
			)}
			<div ref={ref} className={cn(classes.EngineCanvas, {}, [classes.rete, className])}></div>
		</>
	)
}
